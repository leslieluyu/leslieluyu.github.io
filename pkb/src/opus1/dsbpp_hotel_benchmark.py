...
def _SavePodConfig(benchmark_spec, label):
  ...
  with open(filename, 'a') as f:
    ...
    f.write(pod_config)
    f.write("\n" + os.getcwd())
    numainfo =  os.popen(os.getcwd() + "/perfkitbenchmarker/linux_benchmarks/check_pods_numa.sh").read()
    f.write(numainfo)
    f.write("\n================\n")
    info = _GetControlNode(benchmark_spec).RemoteCommandWithReturnCode("/home/ansible/yulu/applications.microservices.performance.tool.patamium/numa_tools/check_pods_numa.sh", ignore_failure=True, suppress_warning=True)[0] 
    f.write(info)

def _SaveNumaPodInfo(benchmark_spec, label):
  filename = vm_util.PrependTempDir('numa_pod_info.txt')
  with open(filename, 'a') as f:
    f.write(f"\n{label}:\n")
    return _GetControlNode(benchmark_spec).RemoteCommandWithReturnCode(' '.join(kubectl_cmd), ignore_failure=suppress_failure, suppress_warning=suppress_warning, should_log=force_info_log, timeout=timeout)
    info =  os.popen("./checkNuma.sh")
    f.write(info)


def _GetWorkerNodeMinNumaCount(benchmark_spec):
  def _RunOnTarget(worker):
    return worker.RobustRemoteCommand("cat /sys/devices/system/node/online")[0]

  def _normalize_core_list(cores, ceil=None):
    r = set()
    core_list_re = re.compile(r'^\d+$|^\d+-\d+$')
    for item in cores.split(','):
        item = item.strip()
        if not core_list_re.match(item):
            msg = "'{}' is not valid, e.g. '1,2', '1,3-4,7'.".format(cores)
            raise ValueError(msg)
        if '-' in item:
            l, h = map(int, item.split('-'))
            (l, h) = sorted([l, h])
            if ceil and h > ceil:
                msg = "'{}' is not valid, exceeding the max {}".format(cores,
                                                                       ceil)
                raise ValueError(msg)
            r |= set(range(l, h+1))
        else:
            if ceil and int(item) > ceil:
                msg = "'{}' is not valid, exceeding the max {}".format(cores,
                                                                       ceil)
                raise ValueError(msg)
            r.add(int(item))
  ...
  worker_outputs = vm_util.RunThreaded(_RunOnTarget, _GetWorkers(benchmark_spec))
  numa_nodes_count = []

  for output in worker_outputs:
    online_nodes = _normalize_core_list(output)
    numa_nodes_count.append(len(online_nodes))
  return min(numa_nodes_count)
...
def Prepare(benchmark_spec):
  ...
  def _PrepareCluster(benchmark_spec):
    ...
    def _TweakManifests(benchmark_spec):
      need_anti_affinity = False
      if FLAGS.dsbpp_hotel_enable_crirm and FLAGS.dsbpp_hotel_namespace_count <= _GetWorkerNodeMinNumaCount(benchmark_spec):
        need_anti_affinity = True

      vm_util.RunThreaded(_TweakInstanceManifests,
                           [((inst_id, need_anti_affinity), {}) for inst_id in range(FLAGS.dsbpp_hotel_namespace_count)])

    def _TweakInstanceManifests(inst_id, need_anti_affinity=False):
      rootdir = os.path.join(vm_util.GetTempDir(), DSBPP_DIRECTORY, "hotelReservation", "k8s")
      deploydir = os.path.join(vm_util.GetTempDir(), 'dsbpp_deploy{}'.format(inst_id))
    ...
    _TweakManifests(benchmark_spec)
  ...
  #_SaveNumaPodInfo(benchmark_spec, "NumaPodInfo")
...
def Run(benchmark_spec):
  ...
  def _AutoScale(benchmark_spec, workload):
    ...
    while True:
      ...
      if autoscale_util.MeetsExitCriteria(throughputs, times=6):
